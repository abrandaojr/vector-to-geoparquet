...
Geographic scope
----------------
Output CRS  : SIRGAS 2000 geographic -- EPSG:4674 (degrees, GeoParquet standard)
Metric CRS  : SIRGAS 2000 / Brazil Polyconic -- EPSG:5880 (metres, used
              internally for tile IDs and Hilbert ordering only)
...
# Hilbert distance: Shapely >= 2.0 vectorised API; pure-Python fallback otherwise.
try:
    from shapely import hilbert_distance as _shapely_hilbert_distance

    def _hilbert(gs, bounds, p):
        return _shapely_hilbert_distance(gs, bounds, p=p)
...
# Output CRS: SIRGAS 2000 geographic (EPSG:4674) -- official Brazilian
# geodetic reference system per IBGE Resolution R.PR-1/2005.
CRS_GEO = "EPSG:4674"

# Internal metric CRS used only for tile IDs and Hilbert curve ordering.
# Tile IDs are meaningless outside Brazil; this CRS is calibrated for
# Brazilian territory.
CRS_METRIC = "EPSG:5880"  # SIRGAS 2000 / Brazil Polyconic
...
def convert_to_geoparquet(
    ...
    Notes
    -----
    Tile IDs (``tile_col``, ``tile_row``, ``tile_id``) and Hilbert curve
    ordering are computed in EPSG:5880 (SIRGAS2000 / Brazil Polyconic,
    unit: metres) for correct metric-distance calculations. Output geometry
    is stored in EPSG:4674 (SIRGAS2000 geographic, unit: degrees) for
    GeoParquet 1.1 interoperability.

    Tile assignment uses each feature's ``representative_point``. For
    polygons that span more than one 25 km tile, ``tile_id`` reflects the
    tile containing the representative point, not the full spatial extent.
    """
    ...
    # ------------------------------------------------------------------
    # 3. Normalise to EPSG:4674 (geographic)
    # ------------------------------------------------------------------
    if gdf.crs is None:
        warnings.warn(
            "No CRS defined -- assuming EPSG:4674.", UserWarning, stacklevel=2
        )
    ...
    # ------------------------------------------------------------------
    # 6. Project to metric CRS for tile IDs and Hilbert ordering.
    #    Output geometry (gdf) stays in EPSG:4674.
    # ------------------------------------------------------------------
    gdf_metric = gdf.to_crs(CRS_METRIC)
    gp_metric = gdf_metric["geometry"]

    rp = gp_metric.representative_point()
    ...
    # ------------------------------------------------------------------
    # 7. Hilbert sort in metric space for spatial locality
    # ------------------------------------------------------------------
    gdf["_h"] = _hilbert(gp_metric, gp_metric.total_bounds, hilbert_p)
    del gdf_metric, gp_metric, rp, tcol, trow
    ...
    # gdf geometry is in EPSG:4674 -- ready for GeoParquet output.
